...
def authenticate():
    ...
    if method == "password":
        ...
        try:
            # For Windows, use Windows Hello directly
            if verify_windows_pin(None):
                response = make_response(
                    jsonify({"message": "Windows Hello authenticated!"}), 200)
                response.set_cookie(
                    "session_token", MASTER_PASSWORD, httponly=True, secure=True)
                return response
            return jsonify({"error": "Windows Hello authentication failed"}), 401
        except Exception as e:
            print(f"Windows Hello authentication error: {str(e)}")
            return jsonify({"error": "Authentication failed"}), 401
