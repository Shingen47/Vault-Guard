...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    // Handle storing credentials
    else if (message.action === "storeCredentials") {
        console.log("Background script: Received request to store credentials");
        
        // Get the credentials from the message
        const credentials = message.credentials;
        console.log("Background script: Received credentials for website:", credentials.website);
        ...
        // In production, use a proper encryption library like the Web Crypto API
        ...
        console.log("Background script: Sending encrypted credentials to backend");
        
        // Use XMLHttpRequest to send the credentials to the backend
        const xhr = new XMLHttpRequest();
        xhr.open("POST", "http://127.0.0.1:5000/store-credentials", true);
        xhr.setRequestHeader("Content-Type", "application/json");
        
        xhr.onload = function() {
            console.log("Background script: Store credentials response status:", xhr.status);
            if (xhr.status === 200) {
                try {
                    const data = JSON.parse(xhr.responseText);
                    console.log("Background script: Store credentials response:", data);
                    
                    if (data.success) {
                        console.log("Background script: Credentials stored successfully");
                        sendResponse({ success: true, message: "Credentials stored successfully" });
                    } else {
                        console.error("Background script: Failed to store credentials:", data.error);
                        sendResponse({ success: false, error: data.error || "Failed to store credentials" });
                    }
                } catch (e) {
                    console.error("Background script: Error parsing store credentials response:", e);
                    sendResponse({ success: false, error: "Error parsing response from server" });
                }
            } else {
                console.error("Background script: Error response from backend:", xhr.status);
                sendResponse({ success: false, error: "Error response from backend: " + xhr.status });
            }
        };
        
        xhr.onerror = function() {
            console.error("Background script: Network error");
            sendResponse({ success: false, error: "Failed to connect to the backend" });
        };
